    ...
    >
        ...
        <el-table :data="rows" size="mini" stripe v-loading="loading" border :header-cell-style="{background:'#495057',color:'#fff'}">
            ...
            <el-table-column prop="login-at" align="center" width="80" :label="'col.login-at' | pt">
                <template slot-scope="{row}">
                    <div class="font-monospace">{{ row.loginAt }}</div>
                </template>
            </el-table-column>
            <el-table-column
                prop="option"
            ...
            >
                <template slot-scope="{row}">
                    <el-button size="mini" @click.prevent="logout(row)">{{ 'logout' | pt }}</el-button>
                </template>
            </el-table-column>
        </el-table>
    </el-dialog>
...
const pt = prefixTransaction('component.socket.online-members-dialog');

@Component({
    filters: { pt },
})
export default class OnlineMembersDialog extends Vue {
    public pt = pt;
    public visible = false;
    public fid = 0;

    public rows: Pr.Member[] = [];
    ...
    public reload() {
        ...
        actions.socket
            .getOnlineMembers(this.fid)
            .then((members) => {
                const data = { members };

                return Ajax.post(ApiPath.ApiSocketOnlineMembers, { data })
                    .then((res) => {
                        const maps: { [id: number]: any } = {};
                        res.data[0].forEach((o: any) => (maps[o.id] = o));
                        res.data[1].forEach((o: any) => (maps[o.id] = o));
                        this.rows = members.map((mem, indx) => {
    ...
    public logout(row: Pr.Member) {
        const label = pt('confirm-logout-user', { name: `${row.account}【${row.name}】` });
        this.$confirm(label)
            .then(() => {
                actions.socket.logoutMember(row.id, row.token).then(() => {
                    this.reload();
                });
            })
            .catch(() => {
