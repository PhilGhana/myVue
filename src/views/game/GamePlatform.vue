...
export default class GamePlatform extends Vue {
    public pt = pt;
    public $refs!: {
        grid: ListGrid;
    ...
    public reload(page?: number) {
        ...
        this.response = Ajax.get(ApiPath.ApiGamePlatformList, { data })
            .then((res) => {
    ...
    public edit(row: GamePlatformItem) {
        this.$refs.dialog.open({
            ...
            enabled: row.enabled,
            syncReportDelay: row.syncReportDelay,
            setting: row.setting,
            order: row.order,
    ...
    public toggleEnabled(row: GamePlatformItem, enabled: boolean) {
        const data = { id: row.id, enabled: enabled ? 1 : 0 };
        row.promiseEnabled = Ajax.post(ApiPath.ApiGamePlatformToggleEnabled, { data }).then(() => {
            row.enabled = enabled;
            const maintain = row.maintain ? 1 : 0;
